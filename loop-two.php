<nav class="post-articles">
    ...
    <div class="row list-articles left">
        <?php 
            $args = array( 'post_type' => 'artigos', 'posts_per_page' => 4, 'orderby' => 'date', 'offset' => 4 ); 
    ...
    <!-- PROJETOS -->
    <div class="list-projects row left small-pull-2">
        <?php 
            $args = array( 'post_type' => 'projetos', 'posts_per_page' => 3, 'orderby' => 'date', 'offset' => 3 ); 
            $loop = new WP_Query( $args );
            while ( $loop->have_posts() ) : $loop->the_post();
    ...
    <!-- PENSADOURO -->
    <div class="list-thought row left small-pull-1">
        <?php       
            $category_id = get_cat_ID( 'Pensadouro' );
            query_posts('showposts=4&category_name=pensadouro&offset=4'); 
